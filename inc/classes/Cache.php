...
EOS;
        ...
        if (akina_option('live_search_comment')) {
            ...
            foreach ($comments as $comment) {
                ...
                $output[] = array(
                    ...
                    "comments" => "",
                    "text" => $is_private ? ($comment->comment_author . ": " . __('The comment is private', 'Saki')) : str_replace($vowels, ' ', preg_replace($regex, ' ', $comment->comment_author . "：" . $comment->comment_content))
                );
            }
        }
        return $output;
    ...
    public static function update_database() {
        ...
        if ($manifest) {
            ...
            $wpdb->query("DELETE FROM  $Saki_table_name WHERE `mate_key` ='manifest_json'");
            $wpdb->query("DELETE FROM  $Saki_table_name WHERE `mate_key` ='json_time'");
