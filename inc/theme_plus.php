...
function poi_time_since( $older_date, $comment_date = false, $text = false ) {
  $chunks = array(
    array( 24 * 60 * 60, __( ' days ago', 'Saki' ) ),/*天前*/
    array( 60 * 60 , __( ' hours ago', 'Saki' ) ),/*小时前*/
    array( 60 , __( ' minutes ago', 'Saki' ) ),/*分钟前*/
    array( 1, __( ' seconds ago', 'Saki' ) )/*秒前*/
  );
 ...
 * Ajax评论
 */
if ( version_compare( $GLOBALS['wp_version'], '4.4-alpha', '<' ) ) { wp_die(__('Please upgrade wordpress to version 4.4+','Saki')); }/*请升级到4.4以上版本*/
// 提示
            ...
            <div class="main shadow">
                ...
                <div class="commentinfo">
                  <section class="commeta">
                    <div class="left">
                      <h4 class="author"><a href="<?php comment_author_url(); ?>"><?php echo get_avatar( $comment->comment_author_email, '80', '', get_comment_author() ); ?><?php comment_author(); ?> <span class="isauthor" title="<?php esc_attr_e('Author', 'Saki'); ?>"></span></a></h4>
                    </div>
                    <div class="right">
...
function Exuser_center(){ ?>
  <script language='javascript' type='text/javascript'> 
    ...
    function gopage(url,type){ 
        URL = url; 
        if(type == 1){
          TYPE = <?php _e('dashboard','Saki')/*管理后台*/?>;
        }else{
          TYPE = <?php _e('home','Saki')/*主页*/?>;
        }
    ...
    function doUpdate(num){ 
        document.getElementById('login-showtime').innerHTML = '<?php _e("Login successful, ","Saki")/*空降成功*/?>'+num+'<?php _e("seconds later automatically transfer to","Saki")/*秒后自动转到*/?>'+TYPE; 
...
function login_ok(){ 
  ...
  wp_get_current_user();
?>
  <p class="ex-login-avatar"><a href="http://cn.gravatar.com/" title="<?php _e('Change avatar','Saki')/*更换头像*/?>" target="_blank" rel="nofollow"><?php echo get_avatar( $current_user->user_email, '110' ); ?></a></p>
  <p class="ex-login-username"><?php _e('Hello, ','Saki')/*你好，*/?><strong><?php echo $current_user->display_name; ?></strong></p>
  <?php if($current_user->user_email){echo '<p>'.$current_user->user_email.'</p>';} ?>
  <p id="login-showtime"></p>
  <p class="ex-logout">
    <a href="<?php bloginfo('url'); ?>" title="<?php _e('Home','Saki')/*首页*/?>"><?php _e('Home','Saki')/*首页*/?></a>
    <?php if(current_user_can('level_10')){  ?>
    <a href="<?php bloginfo('url'); ?>/wp-admin/" title="<?php _e('Manage','Saki')/*后台*/?>" target="_top"><?php _e('Manage','Saki')/*后台*/?></a> 
    <?php } ?>
    <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" title="<?php _e('Logout','Saki')/*登出*/?>" target="_top"><?php _e('Sign out? ','Saki')/*登出？*/?></a>
  </p>
<?php 
}
...
function the_headPattern(){
  ...
  if(is_single()){
    ...
    $t .= '<h1 class="entry-title search-title"> '.sprintf( __( "Search results for \" %s \"","Saki" ), get_search_query()) ./*关于“ '.get_search_query().' ”的搜索结果*/'</h1>';
  }
...
function the_video_headPattern_hls(){
  ...
  <div class="pattern-center <?php if(is_single()){echo $center;} ?>">
    ...
    <style>.pattern-center::before,.pattern-center-Saki::before{display:none}</style>
    <header class="pattern-header <?php if(is_single()){echo $header;} ?>"><?php echo $t; ?></header>
  </div>
  <?php else :
    echo '<div class="blank"></div>';
...
function the_video_headPattern_normal(){
  ...
  if(is_single()){
    ...
    global $user_ID; 
    if($user_ID && current_user_can('level_10')) {
        $edit_this_post_link = '<span class="bull">·</span><a href="'.get_edit_post_link().'">'._e("EDIT","Saki").'</a>';
    } else {
        $edit_this_post_link = '';
    }
    $t .= the_title( '<h1 class="entry-title">', '<button id="coverVideo-btn" class=".constant-width-to-height-ratio" onclick="coverVideo()"><i class="fa fa-pause" aria-hidden="true"></i></button></h1>', false);
    ...
    <div class="header-user-avatar">
      ...
      <div class="header-user-menu">
        ...
        <div class="user-menu-option">
          <?php if (current_user_can('level_10')) { ?>
            <a href="<?php bloginfo('url'); ?>/wp-admin/" target="_blank"><?php _e('Dashboard','Saki')/*管理中心*/?></a>
            <a href="<?php bloginfo('url'); ?>/wp-admin/post-new.php" target="_blank"><?php _e('New post','Saki')/*撰写文章*/?></a>
          <?php } ?>
          <a href="<?php bloginfo('url'); ?>/wp-admin/profile.php" target="_blank"><?php _e('Profile','Saki')/*个人资料*/?></a>
          <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" target="_top"><?php _e('Sign out','Saki')/*退出登录*/?></a>
        </div>
      </div>
    </div>
  <?php
  }else{ 
    $ava = 'https://cdn.jsdelivr.net/gh/moezx/cdn@3.1.9/img/Saki/images/none.png';
...
function siren_private_message_hook($comment_content , $comment){
    ...
    if ( $is_private ) return '<i class="fa fa-lock" aria-hidden="true"></i> '.__("The comment is private","Saki")/*该评论为私密评论*/;
...
function siren_get_useragent($ua){
  if(akina_option('open_useragent')){
    ...
    $imgurl = 'https://cdn.jsdelivr.net/gh/moezx/cdn@3.2.7/img/Saki/images/ua/svg/';
